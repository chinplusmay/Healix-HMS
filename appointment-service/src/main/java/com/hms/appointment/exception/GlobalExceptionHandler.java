...
import com.hms.appointment.dto.ApiError;
...
public class GlobalExceptionHandler {
    ...
    /** Patient/Doctor referenced in the request body doesn't exist upstream -> 400. */
    @ExceptionHandler(InvalidReferenceException.class)
    public ResponseEntity<ApiError> handleInvalidReference(InvalidReferenceException ex, HttpServletRequest req) {
        return build(HttpStatus.BAD_REQUEST, ex.getMessage(), req);
    }

    /** Illegal lifecycle move (e.g. CANCELLED -> COMPLETED) -> 409. */
    @ExceptionHandler(IllegalStatusTransitionException.class)
    public ResponseEntity<ApiError> handleIllegalTransition(IllegalStatusTransitionException ex, HttpServletRequest req) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), req);
    }

    /** Upstream service down / 5xx / timeout -> 502. */
    ...
    /** Triggered when JSON deserialization fails — e.g. unknown enum value. */
    ...
    /** Triggered when a path/query variable cannot be coerced (e.g. /appointments/abc). */
